...
Page({
  ...
  async toggleFavorite() {
    if (this.data.isFavorited) {
      wx.showLoading({ title: app.t('res_removing_favorite') || '正在移除...', mask: true });
      try {
        if (this.data.historyId) {
          const db = wx.cloud.database();
          await db.collection('histories').doc(this.data.historyId).remove();
        }
        this.setData({
          isFavorited: false,
          historyId: null
        ...
        wx.hideLoading();
        wx.showToast({ title: app.t('res_removed_favorite') || '已移除', icon: 'none' });
      } catch (err) {
        console.error('移除集邮失败:', err);
        wx.hideLoading();
        wx.showToast({ title: app.t('err_remove_failed') || '移除失败', icon: 'none' });
      }
    } else {
      wx.showLoading({ title: app.t('res_adding_favorite') || '正在加入...', mask: true });
      try {
        ...
        wx.hideLoading();
        wx.showToast({ title: app.t('res_added_favorite') || '已加入集邮册', icon: 'success' });
      } catch (err) {
        console.error('保存集邮历史失败:', err);
        wx.hideLoading();
        wx.showToast({ title: app.t('err_add_failed') || '加入失败', icon: 'none' });
      }
    }
  },
